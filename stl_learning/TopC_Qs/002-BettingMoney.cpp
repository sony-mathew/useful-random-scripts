...
Problem Statement for BettingMoney
...
class BettingMoney {
  ...
  public:
    ...
    int moneyMade(vector <int> amounts, vector <int> centsPerDollar, int finalResult) {
      int sum = 0;
      REP(i, amounts.size()) sum = sum + amounts.at(i);
      return (sum - amounts.at(finalResult))*100 - (amounts.at(finalResult)*centsPerDollar.at(finalResult));
    }
};