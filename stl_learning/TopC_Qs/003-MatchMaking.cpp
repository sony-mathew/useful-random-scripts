...
class MatchMaking {
  public:
    int getMatchCount(const char *a, const char *b)
    {
      int r = 0;
      while(*a) 
      {
    ...
    string makeMatch(vector <string> namesWomen, vector <string> answersWomen, vector <string> namesMen, vector <string> answersMen, string queryWoman)
    {
      ...
      {
        int maxMatchCount = 0;
        vps::iterator it, matchIndex;
        for(it = men.begin(); it != men.end(); it++)
        {
          int matchCount = getMatchCount(women.at(i).second.c_str(), it->second.c_str());
        ...
        if (queryWoman == women[i].first) return matchIndex->first;
        men.erase(matchIndex);
      }
      return "";
    }
